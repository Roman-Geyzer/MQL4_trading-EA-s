...
int PlaceBuyOrder()
   {
      ...
      ticket = OrderSend (Symbol() , OP_BUY,Order_size, Ask, 3, Ask-StopLoss*CalculatePipSize() , Ask+TakeProfit*CalculatePipSize(), "HA hourly TP EA", MagicNumber, 0,clrGreen);
...
int PlaceSellOrder()
   {
      ...
      // create buy order
      ticket = OrderSend (Symbol() , OP_SELL,Order_size , Bid , 3, Bid+StopLoss*CalculatePipSize() , Bid-TakeProfit*CalculatePipSize(), "HA hourly TP EA", MagicNumber, 0,clrRed);
      //the following loop check
      if (ticket<0)
            Print("OrderSend failed with error #" , GetLastError());
                 ...
                 if(OrderSymbol() ==Symbol())
                    if(OrderType() == OP_BUY)
                        if(OrderStopLoss()<Bid-Trail*CalculatePipSize())
                            if (OrderModify(OrderTicket(),OrderOpenPrice(),Bid-Trail*CalculatePipSize(),OrderTakeProfit() , 0 , CLR_NONE))
                 ...
                 if(OrderSymbol() ==Symbol())
                    if(OrderType() == OP_SELL)
                       if(OrderStopLoss()>Ask+Trail*CalculatePipSize())
                          if (OrderModify(OrderTicket(),OrderOpenPrice(),Ask+Trail*CalculatePipSize(),OrderTakeProfit() , 0 , CLR_NONE))
                              Print ( "order "+IntegerToString(ticket) + " updates SL to ", OrderStopLoss());
          }
    }
}  
