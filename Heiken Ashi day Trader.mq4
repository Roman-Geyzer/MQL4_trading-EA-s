...
extern int CurrencyRange=100; 
extern int StopLoss=100; 
extern int Trail=200; 
extern int place_trades_time = 8;//hour trades are placed
extern double wik_ratio = 2;//max ratio of allowed wiks - trigger os sale / buy and close
...
int PlaceSellOrder()
   {
      Order_size = CalculateLotSize (CurrencyRange);
...
bool check_is_buy ()
{
      ...
      if (HA_Open[1] < HA_Close[1])
          return (  ((HA_Open[1] - HA_Low[1])*wik_ratio) <(HA_High[1] - HA_Close[1])||(HA_Low[1] == HA_Open[1])  );
      else return (false);
}    
...
bool check_is_sell ()
{
      ...
      for (int i= 14 ; i>=0 ; i--)
      {
       ...
       if( HA_Open[1] > HA_Close[1])
       {
            return ( ((HA_High[1] - HA_Open[1])*wik_ratio) <((HA_Close[1] - HA_Low[1]))|| (HA_High[1] == HA_Open[1] ));
       }
       else return (false);
...
void check_open_trade() // check vurrent open and closes if neccesery
{
   ...
   if(OrderSelect(ticket, SELECT_BY_TICKET))    // select current trade for OrderType function
   {
     if (OrderType()== 0 ) // check if Buy
       {
            if( HA_Open[1] >= HA_Close[1] ||  ((HA_Open[1] - HA_Low[1])*wik_ratio) >=((HA_High[1] - HA_Close[1]))  ) 
            {
               if (Close[1] < High[2])
               {
     ...
     else // must be sell
     {
        if (Close[1] > Low[2])
        {
              if( HA_Open[1] <= HA_Close[1] || ((HA_Close[1] - HA_Low[1])*wik_ratio) <= ((HA_High[1] - HA_Open[1])) )
              {
...
void OnTick()
{
   Print(CalculatePipSize());
    if (TimeHour(TimeCurrent())== place_trades_time)
    {
