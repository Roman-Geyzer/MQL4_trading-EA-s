...
extern int CandlesForTrailing=30;
extern int PipsBufferForTrailing=10;
extern double Trail=50; 
extern double TrailStartPips=50;
extern int TrailStartCandles=50;
...
extern int ExitTradeOnXPipsMove = 80;
extern int CandlesCountForPipsMove = 1;

extern int ExitTradeOnXCandlesOppositeMove = 10;
...
//other filters      
      if (LongMAFilter) StringAdd (other_filters , " *LongMAFilter* ,");
      if (LongSRFilter) StringAdd (other_filters ,StringConcatenate( " *LongSRFilter*, period is: ", LongSRPeriod, " ,slck is: " , SlackForSRFilter)); 
...
//trail
      ...
      else StringAdd (trail_p , StringConcatenate(" *User fixed trail* ,Trail (pips) is: " , Trail));
//TP    
   ...
   if (OpenOrdersThisPair (Symbol()) ==1 && MainRun )
    {
            ...
            if(OrderSelect(o,SELECT_BY_POS,MODE_TRADES))
               if(OrderMagicNumber() == MagicNumber)
                 if(OrderSymbol() ==Symbol())
                 {
                     if (!Trail_Started)
                     {
                        if (TrailStartCandles <= iBarShift(NULL , PERIOD_CURRENT , OrderOpenTime(), false)) 
                        {
                        Print ("candles");
                        Trail_Started = true;
                        ...
                        if(OrderType() == OP_BUY)
                        {
                            if(OrderOpenPrice() <Bid-TrailStartPips*CalculatePipSize()) Trail_Started = true;
                        }  
                        else if(OrderOpenPrice() >Ask+TrailStartPips*CalculatePipSize()) Trail_Started = true;  //for sell
                     }
                     manage_open_trade();
                 }
         }
     ...
     ((Mon && Day_Of_Week == 1) || (Tue && Day_Of_Week == 2) || (Wed && Day_Of_Week == 3) || (Thur && Day_Of_Week == 4) || (Fri && Day_Of_Week == 5) ))
      ...
      MainRun = false;
     if (OpenOrdersThisPair (Symbol()) ==1 && Trail_Started) manage_SL();
}
