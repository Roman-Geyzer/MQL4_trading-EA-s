...
extern double ExitOnRRMulitplier = 10;
...
//TP    
      ...
      " ,Exit On RR Mulitplier  is : ", ExitOnRRMulitplier,      
      ...
      rev = StringConcatenate (" Master Trader V42, main strategy is: " , MainS , "Magic number is: " , MagicNumber);
               ...
               if(OrderMagicNumber() == MagicNumber)
                 if(OrderSymbol() ==Symbol())
                 {
                     if (!Trail_Started)
                     {
                        ...
                        {
                             Print ("Trail strted due to candels, amount is: " , TrailStartCandles , rev);
                             Trail_Started = true;
                        }
                        else if(OrderType() == OP_BUY)
                        {
                            if(OrderOpenPrice() <Bid-TrailStartPips*CalculatePipSize())
                            {
                                  Trail_Started = true;
                                  Print ("Trail strted due to Pips, amount is: " , TrailStartPips , rev);
                            }
                        }  
                        else if(OrderOpenPrice() >Ask+TrailStartPips*CalculatePipSize())//for sell
                           {
                               Trail_Started = true;  
                               Print ("Trail strted due to Pips, amount is: " , TrailStartPips , rev);
                           }
